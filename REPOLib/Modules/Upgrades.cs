...
public static class Upgrades
{
    ...
    internal static void InvokeStartActions(string steamId)
    {
        ...
        {
            if (upgrade.StartAction is null) continue;

            try
            {
                var level = upgrade.GetLevel(steamId);
    ...
    private static void HandleUpgradeEvent(EventData eventData)
    {
        ...
        var upgradeId = (string)data["UpgradeId"];
        var steamId = (string)data["SteamId"];
        var level = (int)data["Level"];

        if (TryGetUpgrade(upgradeId, out var playerUpgrade))
