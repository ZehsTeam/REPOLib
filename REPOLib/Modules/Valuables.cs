...
using UnityEngine;
using ListExtensions = REPOLib.Extensions.ListExtensions;
...
public static class Valuables
{
    ...
    public static IReadOnlyList<GameObject> AllValuables
        => GetValuables();

    /// <summary>
    ///     Gets all valuables registered with REPOLib.
    /// </summary>
    public static IReadOnlyList<GameObject> RegisteredValuables
        => _valuablesRegistered;

    private static IEnumerable<GameObject> PendingAndRegisteredValuables
        => _valuablesToRegister.Keys.Concat(_valuablesRegistered);

    internal static void RegisterInitialValuables()
    {
        if (_initialValuablesRegistered) return;

        Logger.LogInfo("Adding valuables to valuable presets.");
        _valuablesToRegister.Keys.ForEach(RegisterValuableWithGame);
    ...
    private static void RegisterValuableWithGame(GameObject valuable)
    {
        ...
        foreach (string? presetName in presetNames)
        {
            ...
            {
                Logger.LogWarning($"Failed to add valuable \"{valuable.name}\" to valuable preset \"{presetName}\"", true);
            }
        }
    }
    ...
    public static void RegisterValuable(ValuableObject valuableObject, List<string>? presetNames)
    {
        ...
        {
            Logger.LogWarning($"Valuable \"{valuableObject.name}\" does not have any valid valuable preset names set. Adding generic valuable preset name.", true);
    ...
    public static GameObject? SpawnValuable(ValuableObject valuableObject, Vector3 position, Quaternion rotation)
    {
        ...
        Logger.LogInfo($"Spawned valuable \"{gameObject.name}\" at position {position}, rotation: {rotation.eulerAngles}", true);
        return gameObject;
    }
