...
public static class Levels
{
    ...
    public static IReadOnlyList<Level> AllLevels
        => GetLevels();

    /// <summary>
    ///     Get all levels registered with REPOLib.
    /// </summary>
    public static IReadOnlyList<Level> RegisteredLevels
        => _levelsRegistered;

    internal static void RegisterInitialLevels()
    {
        if (_initialLevelsRegistered) return;

        ValuablePresets.CacheValuablePresets();
        Logger.LogInfo("Adding levels.");

        foreach (Level? level in _levelsToRegister)
    ...
    private static void RegisterLevelWithGame(Level level)
    {
        ...
        for (int i = 0; i < level.ValuablePresets.Count; i++)
        {
            ...
            if (ValuablePresets.AllValuablePresets.TryGetValue(valuablePreset.name, out LevelValuables? foundPreset))
            {
                ...
                Logger.LogInfo($"Replaced proxy preset \"{valuablePreset.name}\" in level \"{level.name}\".", true);
            }
            else
            {
                ValuablePresets.RegisterValuablePreset(valuablePreset);
                Logger.LogInfo($"Registered valuable preset \"{valuablePreset.name}\" from \"{level.name}\".", true);
            }
        }

        RunManager.instance.levels.Add(level);
        Logger.LogInfo($"Added level \"{level.name}\"", true);
    ...
    public static void RegisterLevel(Level level)
    {
        ...
        List<(GameObject, ResourcesHelper.LevelPrefabType)> modules = level.GetAllModules()
        ...
        foreach ((GameObject prefab, ResourcesHelper.LevelPrefabType type) in modules)
        {
    ...
    public static Level? GetLevelByName(string? name)
        => GetLevels().FirstOrDefault(level =>
            level.name.EqualsAny([ name, $"Level - {name}" ], StringComparison.OrdinalIgnoreCase));
