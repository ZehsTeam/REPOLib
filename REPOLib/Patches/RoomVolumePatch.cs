...
namespace REPOLib.Patches
{
    ...
    internal class RoomVolumeExample
    {
        ...
        private static void CheckSetPatch(RoomVolumeCheck __instance)
        {
            ...
            if(__instance.inTruck != inTheTruck)
            {
                ...
                {
                    PhotonNetwork.RaiseEvent(NetworkingEvents.ExampleEvent.EventByte, message, NetworkingEvents.RaiseAll, SendOptions.SendReliable);
                    //The same eventbyte can be used if targeting the same method
                    PhotonNetwork.RaiseEvent(NetworkingEvents.ExampleEvent.EventByte, message2, NetworkingEvents.RaiseOthers, SendOptions.SendReliable);
                }
